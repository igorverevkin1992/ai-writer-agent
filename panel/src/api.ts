...
export const OFFLINE_MESSAGE = "Сервер панели недоступен — перезапустите `ugar panel`";
...
export function describeError(status: number, message: string): string {
  ...
  switch (status) {
    ...
    case 500:
      return msg.startsWith("внутренняя ошибка") ? msg : `внутренняя ошибка сервера — подробности в logs/panel.log: ${msg}`;
    default:
      return msg;
  }
}
