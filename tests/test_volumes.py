"""Многотомность (аудит 2, этап 5, п. 27) без ломки тома 1: `chapters/001` и документы без номера тома
работают как раньше; том 2 живёт в `chapters/Т2/`, берёт документы `…_Том2.md`, состояния изолированы;
`ugar volume close/open/status`."""
...
from ugar import apilog, circles, exporter, volume as volume_mod
from ugar.cli import app
from ugar.config import Config, load_config, set_volume
from ugar.fsm import ChapterState, all_states
...
def test_пути_тома_1_как_были(tmp_path):
    ...
    assert ws.volume == 1
    assert ws.chapter_dir(5) == tmp_path / "chapters" / "005"
    assert ws.status_path(5) == tmp_path / "chapters" / "005" / "status.yaml"
    assert ws.chapter_rel(5) == "chapters/005"
    ws2 = ws.for_volume(2)
    assert ws2.volume == 2 and ws2.root == tmp_path
    assert ws2.chapter_dir(1) == tmp_path / "chapters" / "Т2" / "001"
    assert ws2.chapter_rel(1) == "chapters/Т2/001"
    assert ws.chapter_dir(1, volume=3) == tmp_path / "chapters" / "Т3" / "001"
...
def test_состояния_глав_изолированы_по_томам(ws, library):
    ...
    assert st2.state == "не-начато" and st2.path == ws.root / "chapters" / "Т2" / "001" / "status.yaml"
    st2.transition("собрано", "compile")
    st2.transition("сгенерировано", "write")
    assert (ws.root / "chapters" / "Т2" / "001" / "status.yaml").exists()
...
def test_config_volume_и_ctx(ws, library):
    ...
    set_volume(ws, 2)
    text = (ws.root / "config.yaml").read_text(encoding="utf-8")
    assert "library_dir: УГАР_Библиотека" in text and "volume: 2" in text
    assert load_config(ws).volume == 2
    set_volume(ws, 3)
    assert (ws.root / "config.yaml").read_text(encoding="utf-8").count("volume:") == 1
    ...
    assert (ws.root / "chapters" / "Т2" / "001" / "window.md").exists()
    assert not (ws.root / "chapters" / "001").exists()
    r = CliRunner().invoke(app, ["status"])
    assert r.exit_code == 0 and "Том 2" in r.output and "chapters/Т2/" in r.output, r.output
...
def test_панель_отдаёт_том(ws, library):
    set_volume(ws, 2)
    _make_volume2(library)
    from ugar import server
    from ugar.config import library_dir
...
def test_volume_close_проходит_и_open_переключает(ws, library):
    ...
    stats = (ws.root / "manuscript" / "Том1_статистика.md").read_text(encoding="utf-8")
