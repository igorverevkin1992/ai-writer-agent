...
from ugar import adapters, cancel, review, timing, verifier2, writer
...
def test_apply_edits_кодом_без_модели_и_чистый_дифф(ws, monkeypatch):
    ...
    (ws.chapter_dir(1) / "edits.md").write_text(
        "БЫЛО: Вторая фраза.\nСТАЛО: Другая фраза.\n\nБЫЛО: Третья фраза.\nСТАЛО:\n", encoding="utf-8"
    )
    called = []
    ...
    meta = json.loads((ws.chapter_dir(1) / "draft_2.meta.json").read_text(encoding="utf-8"))
...
def test_apply_edits_смешанные_промпт_от_промежуточного_текста(ws, monkeypatch):
    ...
    prompt = (ws.chapter_dir(1) / "apply_edits_prompt.md").read_text(encoding="utf-8")
...
def test_apply_edits_лимит_итераций_не_мешает_правкам_кодом(ws, monkeypatch):
    ...
    st.data["итераций_правок"] = 3
    st._save()
    (ws.chapter_dir(1) / "edits.md").write_text("УКАЗАНИЕ: переписать\n", encoding="utf-8")
    r = runner.invoke(app, ["apply-edits", "1"])
...
def test_переходы_внутри_команды_помечены_задачей(ws, monkeypatch):
    ...
    (ws.chapter_dir(1) / "edits.md").write_text("", encoding="utf-8")
...
def test_сегодняшнее_авторское_время_по_всем_главам(ws):
    ...
    ws.status_path(4).write_text("", encoding="utf-8")  # битый status.yaml не роняет сводку
    assert timing.today_author_minutes(ws) == 15.0
...
def test_verify2_повторно_без_ключа_и_вручную(ws, monkeypatch):
    ...
    r = runner.invoke(app, ["verify2", "1", "--повторно"])
    assert r.exit_code == 2, r.output
    assert (ws.chapter_dir(1) / "verify2_повторно_prompt.md").exists()
    assert "Другая фраза." in (ws.chapter_dir(1) / "verify2_повторно_prompt.md").read_text(encoding="utf-8")
    assert not (ws.chapter_dir(1) / "flags_повторно.json").exists()
    assert ChapterState(ws, 1).state == "правки"
    # ручной режим: голый список флагов → раздел в review.md; flags.json и решения не тронуты
    flags_before = (ws.chapter_dir(1) / "flags.json").read_text(encoding="utf-8")
    (ws.chapter_dir(1) / "flags_повторно.json").write_text(
        json.dumps([{
    ...
    assert (ws.chapter_dir(1) / "flags.json").read_text(encoding="utf-8") == flags_before
    assert review.load_resolutions(ws, 1) == []
    review_md = (ws.chapter_dir(1) / "review.md").read_text(encoding="utf-8")
    ...
    draft_k, flags = verifier2.load_flags_again(ws, 1)
    assert draft_k == 2 and len(flags) == 1
...
def test_write_варианты_без_ключа(ws, monkeypatch):
    ...
    assert r.exit_code == 2, r.output
    assert "draft_1.alt1.md" in r.output and "--manual --варианты 2" in r.output
    ...
    assert r.exit_code == 1 and "draft_1.alt1.md" in r.output + (r.stderr or "")
    (ws.chapter_dir(1) / "draft_1.alt1.md").write_text("Иная первая фраза. Иная вторая фраза. Третья.\n", encoding="utf-8")
    r = runner.invoke(app, ["write", "1", "--manual", "--варианты", "2"])
    assert r.exit_code == 0, r.output
...
def test_write_варианты_с_фейковым_писателем_и_выбор(ws, monkeypatch):
    ...
    assert (ws.chapter_dir(1) / "draft_1.alt1.md").read_text(encoding="utf-8").startswith("Вариант Б")
    meta_alt = json.loads((ws.chapter_dir(1) / "draft_1.alt1.meta.json").read_text(encoding="utf-8"))
    ...
    assert not (ws.chapter_dir(1) / "verdict.json").exists()  # Э1 главы не запускался, FSM не менялся
    assert "варианты: 2" in st.data["история"][-1]["команда"]

    # выбор варианта: alt1 → draft_1.md, прежний основной — alt0; состояние прежнее, обратимо
    r = runner.invoke(app, ["write", "1", "--выбрать", "alt1"])
    assert r.exit_code == 0, r.output
    assert ws.draft_path(1, 1).read_text(encoding="utf-8").startswith("Вариант Б")
    assert (ws.chapter_dir(1) / "draft_1.alt0.md").read_text(encoding="utf-8").startswith("Вариант А")
    meta = json.loads((ws.chapter_dir(1) / "draft_1.meta.json").read_text(encoding="utf-8"))
