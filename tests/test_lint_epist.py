...
from ugar import guard, lint, lint_epist
from ugar.paths import Workspace
from ugar.schemas import LintReport, Plant
...
def test_тайна5_персонажи_знают_против_матрицы(real):
    """Расхождения реального канона (аудит 7.2): Т-04, Т-03, Т-06 — имя есть в матрице и нет в
    реестре; Т-08 — имя в реестре без главы рядом с именами с главами."""
...
def test_закл7_две_главы_через_или(real):
    """З-02 «Гл. 29 или 40» — открытое решение автора, а не две главы."""
...
def test_реальная_библиотека_новых_ошибок_нет(real):
    ...
    assert {"МАТР-2", "МАТР-3", "ТАЙНА-5", "ЗАКЛ-5", "ЗАКЛ-7", "ПРОЗА-4"} <= codes
