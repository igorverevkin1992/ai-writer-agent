...
from ugar import cancel, review, server, verifier2
...
def test_500_пишет_трейсбек_в_лог(panel, ws, monkeypatch):
    ...
    assert code == 500 and "внутренняя ошибка" in data["error"] and "panel.log" in data["error"]
    log = (ws.logs / "panel.log").read_text(encoding="utf-8")
