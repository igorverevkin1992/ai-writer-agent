...
from ugar import adapters, backup, gitops
...
def test_doctor_библиотека_не_под_git(ws):
    assert backup.layout(ws.root / "УГАР_Библиотека", ws.root).kind == "no-git"
...
def test_doctor_библиотека_внутри_репозитория_кода(ws, library):
    # рабочая область = репозиторий кода конвейера (pyproject + ugar/), библиотека — его подпапка
    (ws.root / "pyproject.toml").write_text("[project]\nname='x'\n", encoding="utf-8")
    (ws.root / "ugar").mkdir()
    _init_repo(ws.root)
    lay = backup.layout(library, ws.root)
    assert lay.kind == "shared" and lay.code_repo and lay.prefix == "УГАР_Библиотека/" and lay.ok is None
    r = runner.invoke(app, ["doctor"])
    assert r.exit_code == 0, r.output
...
def test_library_split_показать_ничего_не_меняет(ws, library):
    ...
    assert not (ws.root.parent / "УГАР_Библиотека").exists()
    assert (ws.root / "config.yaml").read_text(encoding="utf-8") == "library_dir: УГАР_Библиотека\n"
...
def test_library_split_переносит_и_настраивает(ws, library, tmp_path):
    ...
    # config.yaml указывает на новое место, конвейер его находит
    cfg_text = (ws.root / "config.yaml").read_text(encoding="utf-8")
    assert 'library_dir: "куда/Библиотека"' in cfg_text  # относительно рабочей области
    from ugar.config import library_dir, load_config
    ...
    assert "/УГАР_Библиотека/" in (ws.root / ".gitignore").read_text(encoding="utf-8")
...
def test_library_split_отказы_до_действий(ws, library, tmp_path):
    ...
    assert library.exists() and not (ws.root.parent / "УГАР_Библиотека").exists()
...
def test_backup_архив_создаёт_zip_и_ротирует(ws, library, tmp_path):
    ...
    (ws.chapter_dir(1) / "draft_1.md").write_text("текст", encoding="utf-8")
    ...
    assert all(p.name.startswith("УГАР_рабочая_область_") and p.suffix == ".zip" for p in archives)
    names = zipfile.ZipFile(archives[-1]).namelist()
    assert "chapters/001/draft_1.md" in names and "config.yaml" in names
    assert any(n.startswith("regression/golden/") for n in names)
...
def test_архив_после_canonize_apply_если_задан_backup_dir(ws, library, tmp_path):
    _init_repo(library)
    dest = tmp_path / "авто"
    (ws.root / "config.yaml").write_text(f"library_dir: УГАР_Библиотека\nbackup_dir: '{dest.as_posix()}'\n", encoding="utf-8")
    _accepted_chapter(ws, library, 1)
...
def test_backup_добавить_remote_папка(ws, library, tmp_path):
    _init_repo(library)
    bare = tmp_path / "внешний_диск" / "УГАР_Библиотека.git"
...
def test_version():
    ...
    assert r.exit_code == 0 and r.output.startswith("ugar ") and r.output.strip().count(".") >= 2
    import os

    env = {**os.environ, "PYTHONPATH": str(Path(__file__).resolve().parent.parent), "PYTHONUTF8": "1"}
    out = subprocess.run([sys.executable, "-m", "ugar", "-V"], capture_output=True, text=True, encoding="utf-8", env=env)
    assert out.returncode == 0 and out.stdout.startswith("ugar ")
...
def test_thinking_config_писателя_пробрасывается_как_есть():
    """config.yaml корня репозитория задаёт thinking_config в форме google-genai; адаптер отдаёт params без правок."""
    import yaml

    root_cfg = yaml.safe_load((Path(__file__).resolve().parent.parent / "config.yaml").read_text(encoding="utf-8"))
