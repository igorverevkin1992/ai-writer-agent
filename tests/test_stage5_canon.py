...
from ugar import canonchange, exporter, gitops, guard, server
...
def test_canon_change_с_коммитом(ws, library):
    ...
    assert result.lint is not None and (ws.logs / "lint.json").exists()  # линт прошёл по свежим выгрузкам
...
def test_canon_change_без_коммита_даёт_незакоммичено(ws, library):
    ...
    # затем коммит поверх правок автора (сценарий Б, `ugar canon-commit`): writer пуст, чистый git не требуется
...
def test_экспорт_не_перезаписывает_неизменённые_файлы(ws, library):
    ...
    assert "corpus/.index.json" in before and "manifest.json" in before
    ...
    assert changed == {f"corpus/{prose.stem}.txt", "corpus/.index.json", "manifest.json"}, changed
...
def test_кэш_state_видит_правку_status_yaml_на_диске(ws, library, monkeypatch):
    ...
    try:
        ...
        assert s2["chapters"][1]["draft"] == 1 and s2["chapters"][1]["next"] == "ugar verify1 2"
        ...
        assert api.state()["chapters"][1]["state"] == "сгенерировано"
        # регрессия: значение кэшируется и совпадает с regression.is_green; смена config.yaml — пересчёт
