...
def test_повторный_apply_после_сбоя_не_дублирует_записи(ws, library):
    ...
    batch = ws.chapter_dir(1) / "canon_batch.md"
    ...
    # сбой между коммитом и сменой состояния: status.yaml остался «принято»
...
def test_битый_status_не_роняет_панель(ws, library):
    ...
    with pytest.raises(StatusFileError, match="повреждён"):
        ChapterState(ws, 1)
    (ws.root / "regression").mkdir(exist_ok=True)
    (ws.root / "regression" / "report.json").write_text("{битый", encoding="utf-8")
    ...
    assert by[1]["state"] == "повреждено" and "status.yaml" in by[1]["next"]
