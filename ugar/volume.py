...
Рабочая область ведёт один текущий том (`config.yaml: volume`, `Workspace.volume`):
главы тома 1 — `chapters/001`, тома N ≥ 2 — `chapters/ТN/001`; выгрузки `exports/` —
...
рукопись `manuscript/ТомN.md` (+ `.docx`, если установлен python-docx) → статистика тома.
...
def build_manuscript(ws: Workspace, library: Path, volume: int) -> tuple[Path, Path | None, str | None]:
    """Собирает `manuscript/ТомN.md` (главы по порядку с заголовками) и, если установлен python-docx,
    `manuscript/ТомN.docx`. Возвращает (md, docx | None, подсказка | None)."""
    ...
    try:
        ...
        return md, None, "python-docx не установлен — собран только .md; `pip install 'ugar-pipeline[docx]'` даст и .docx"
...
def _verdict(ws: Workspace, chapter: int) -> Verdict | None:
    path = ws.chapter_dir(chapter) / "verdict.json"
    if not path.exists():
        return None
    try:
...
class VolumeStats:
    ...
    cost: float                    # $ по logs/api.jsonl за главы тома
    calls: int
...
def _cost_of_volume(ws: Workspace, volume: int, chapters: set[int]) -> tuple[float, int]:
    """Стоимость по `logs/api.jsonl`: строки с `volume == N`; старые строки без поля «volume» — том 1."""
    cost, calls = 0.0, 0
...
def close_volume(ws: Workspace, cfg: Config, library: Path, volume: int, *, again: bool = False,
    ...
    Выгрузки должны быть тома `volume` (закрывается текущий том; другой — сначала `ugar volume open N`)."""
    ...
    if volume != ws.volume:
        raise RuntimeError(
            ...
            f"переключитесь: `ugar volume open {volume}`."
        )
    ...
    if pending:
        raise RuntimeError(
            ...
            "Доведите главы до «зафиксировано» (`ugar canonize N --apply`)."
        )
    ...
    if doc.exists() and not again:
        raise RuntimeError(
            f"снапшот {doc.name} уже есть в библиотеке; чтобы переписать его — `ugar volume close {volume} --заново`."
        )
    ...
    text = draft.read_text(encoding="utf-8").replace(
        ...
        f"Внесён в канон командой `ugar volume close {volume}` (реестр 3.5, срез мира на конец тома).",
    )
