...
def _save_draft(ws: Workspace, chapter: int, k: int, text: str, cfg: Config, mode: str, extra: dict | None = None,
                suffix: str = "") -> None:
    """draft_k.md + draft_k.meta.json; `suffix` («.alt1») — вариант A/B рядом с основным черновиком."""
    chdir = ws.chapter_dir(chapter)
    guard.write_text(chdir / f"draft_{k}{suffix}.md", text)
    meta = {
    ...
    if extra:
        meta.update(extra)
    guard.write_text(
        chdir / f"draft_{k}{suffix}.meta.json",
...
def write_variants(ws: Workspace, cfg: Config, chapter: int, k: int, n: int) -> list[str]:
    """A/B: n вызовов Писателя по одному окну → draft_k.md, draft_k.alt1.md … (+ meta).
...
def variant_path(ws: Workspace, chapter: int, k: int, label: str):
    return ws.chapter_dir(chapter) / f"draft_{k}{variant_suffix(label)}.md"
...
def existing_variants(ws: Workspace, chapter: int, k: int) -> list[str]:
    ...
    alts = sorted(
        (p.name[len(f"draft_{k}."):-3] for p in chdir.glob(f"draft_{k}.alt*.md")),
...
def choose_variant(ws: Workspace, chapter: int, k: int, label: str) -> None:
    """Выбор автора: вариант label становится draft_k.md; прежний основной сохраняется как alt0
    ...
    if main.exists() and label != "alt0" and not alt0.exists():
        ...
        main_meta = ws.chapter_dir(chapter) / f"draft_{k}.meta.json"
        if main_meta.exists():
            guard.write_text(ws.chapter_dir(chapter) / f"draft_{k}.alt0.meta.json", main_meta.read_text(encoding="utf-8"))
    guard.write_text(main, src.read_text(encoding="utf-8"))
    src_meta = ws.chapter_dir(chapter) / f"draft_{k}{variant_suffix(label)}.meta.json"
    ...
    guard.write_text(ws.chapter_dir(chapter) / f"draft_{k}.meta.json", json.dumps(meta, ensure_ascii=False, indent=2) + "\n")
...
def apply_edits_locally(ws: Workspace, cfg: Config, chapter: int, draft_k: int, edits: list[Edit],
                        new_k: int | None = None) -> tuple[int, LocalEdits]:
    """Правки кодом от черновика draft_k (база приёмки, FR-E3): новый draft_{new_k} с mode «правки (код)».
    ...
    base_text = ws.draft_path(chapter, draft_k).read_text(encoding="utf-8")
    ...
    if not result.needs_model:
        _save_draft(ws, chapter, new_k, result.text, cfg, mode=MODE_LOCAL,
                    extra={"применено_кодом": [e.seq for e in result.applied], "база": draft_k})
    return new_k, result


def edit_prompt(ws: Workspace, chapter: int, draft_k: int, edits: list[Edit], draft_text: str | None = None) -> str:
    """FR-W2: принятый черновик + правки + инструкция «внести точно» (шаблон в templates/).
    `draft_text` — промежуточный текст после правок кодом (Р-023); без него — сам draft_k."""
    ...
    draft = draft_text if draft_text is not None else ws.draft_path(chapter, draft_k).read_text(encoding="utf-8")
    env = Environment(undefined=StrictUndefined)
    return env.from_string(tpl).render(edits=edits, draft=draft)


def apply_edits(ws: Workspace, cfg: Config, chapter: int, draft_k: int, edits: list[Edit], new_k: int | None = None,
    ...
    prompt = edit_prompt(ws, chapter, draft_k, edits, draft_text=base_text)
