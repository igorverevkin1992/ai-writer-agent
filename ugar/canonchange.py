...
ЛЮБАЯ запись в `УГАР_Библиотека/` проходит через `canon_change()`:
...
правка документа и исправление линтера из панели, `ugar canon-commit` — все идут здесь.
...
def check_git(library: Path, *, commit: bool, require_clean: bool = True, action: str = "изменение канона") -> bool:
    ...
    if commit and require_clean and not clean:
        raise RuntimeError(
            ...
            "Закоммитьте их (`ugar canon-commit`) или откатите (`git restore .`), затем повторите."
        )
...
def canon_change(
    ...
    * `writer` выполняется внутри `guard.canon_write_session()` и пишет через `guard.write_text`/`append_text`;
      `writer=lambda: None` — изменения уже на диске (автор правил документы сам, `ugar canon-commit`).
    ...
    * `require_clean=False` — коммит поверх незакоммиченных правок автора допустим (сценарий Б).
    ...
    result.message = (
        f"{UNCOMMITTED_TEXT} ({len(result.dirty_files)} файл(ов)) — закоммитьте их (`ugar canon-commit` или "
