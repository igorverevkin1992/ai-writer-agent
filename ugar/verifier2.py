...
def run_taste(ws: Workspace, cfg: Config, chapter: int, draft: int) -> list[Flag]:
    """Советы по вкусу — отдельный файл taste.json: приёмку не блокируют, в flags.json не попадают."""
    system, user = build_taste_prompt(ws, chapter, draft)
    prompt_path = ws.chapter_dir(chapter) / "taste_prompt.md"
    guard.write_text(prompt_path, f"<!-- system -->\n{system}\n\n<!-- user -->\n{user}\n")
    ...
    guard.write_text(
        ws.chapter_dir(chapter) / "taste.json",
...
def load_taste(ws: Workspace, chapter: int) -> list[Flag]:
    path = ws.chapter_dir(chapter) / "taste.json"
    if not path.exists():
        return []
