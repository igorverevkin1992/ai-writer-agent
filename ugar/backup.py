...
библиотеки целиком (содержимое документов не меняется) и правит config.yaml/.gitignore
        ...
        if self.kind == "shared":
            if self.code_repo:
                return (
                    ...
                    "Переезд: `ugar library-split --показать` (план), затем `ugar library-split`."
                )
            return (
                "коммиты канона перемешаны с другими файлами репозитория; откат и бэкап канона теряют точность. "
                "Переезд: `ugar library-split --показать`, затем `ugar library-split`."
            )
        if self.kind == "no-git":
            return "git init внутри библиотеки (версионирование канона, §5.1) или `ugar library-split` — отдельный репозиторий"
        return ""


def _is_code_repo(top: Path) -> bool:
    return (top / "pyproject.toml").exists() and (top / "ugar").is_dir()
...
class SplitPlan:
    ...
    layout: Layout
    with_history: bool
    library_dir_value: str          # что запишем в config.yaml
    ...
    def lines(self) -> list[str]:
        out = [
            ...
            f"3. config.yaml: library_dir: \"{self.library_dir_value}\"",
        ]
        ...
        if self.fixed_chapters:
            ...
            out.append(
                f"5. status.yaml глав {chapters}: SHA коммита приёмки "
...
def plan_split(ws: Workspace, cfg: Config, lib: Path, target: Path | None = None, *, with_history: bool = False) -> SplitPlan:
    ...
    if lay.kind == "shared":
        if gitops.dirty(lib):
            raise SplitError("в библиотеке незакоммиченные изменения — сначала `ugar canon-commit`, затем переезд.")
    ...
    if not (gitops.identity_available(ws.root) or cfg.commit_author):
        raise SplitError("нет авторства git для первого коммита: git config --global user.email/user.name или commit_author в config.yaml (Д-8).")
    entry = None
...
def _append_gitignore(top: Path, entry: str) -> bool:
    ...
    if text and not text.endswith("\n"):
        text += "\n"
    text += f"# библиотека канона вынесена в отдельный репозиторий (ugar library-split)\n{entry}\n"
    guard.write_text(path, text)
    return True
...
def split_library(ws: Workspace, cfg: Config, plan: SplitPlan) -> list[str]:
    ...
    сначала новый репозиторий, потом config.yaml (с этого момента конвейер смотрит на новое место)."""
    notes: list[str] = []
    ...
    if plan.with_history:
        ...
        commit = gitops.commit_all(target, "Библиотека канона: отдельный репозиторий (ugar library-split)", author=cfg.commit_author)
    ...
    notes.append(f"config.yaml: library_dir: \"{plan.library_dir_value}\"")
...
def after_split_advice(plan: SplitPlan) -> list[str]:
    ...
    out.append("Добавьте удалённые копии (NFR-6, ≥ 2): `ugar backup --добавить-remote <имя> <url|папка>`; проверка — `ugar doctor`.")
    return out
