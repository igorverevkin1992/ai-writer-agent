...
Тонкая обёртка над ядром `ugar/steps/*` (аудит 2, п. 30): здесь только регистрация команд typer
...
from .steps.common import NEXT_STEP as NEXT_STEP  # noqa: F401 — совместимость: `from ugar.cli import NEXT_STEP`
...
def _opt(value, default):
    ...
    такие значения считаем неуказанными. Ядру (`ugar/steps`) это не нужно: его функции вызываются
...
def _friendly(fn):
    ...
    трейсбека. UGAR_DEBUG=1 — полный трейсбек для программных ошибок (2.11).
