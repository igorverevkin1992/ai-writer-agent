...
`StepError` (ugar/errors.py), а также `cancel.Cancelled`, `fsm.TransitionError`, `fsm.StatusFileError`,
`mdparse.MarkupError`. Подтверждения — параметр `yes` плюс `confirm: Callable[[str], bool] | None`.
`ugar/cli.py` (typer) и `ugar/server.py` (панель) — тонкие обёртки: разбор опций → вызов ядра →
...
# ожидаемые ошибки шага (нет файла, структура MD, недопустимый переход FSM, битый status.yaml, …):
