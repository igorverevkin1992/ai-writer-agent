...
Логика тома — в ugar/volume.py; здесь только шаги команд."""
...
def volume_close(
    ...
    if missing:
        secho(
            ...
            + f", затем `ugar volume open {nxt}`.", fg=colors.YELLOW,
        )
        return res
    if next_volume is None:
        next_volume = bool(confirm and confirm(f"Переключить рабочую область на том {nxt} (config.yaml: volume)?"))
    if next_volume:
        set_volume(ws, nxt)
        ...
        echo(f"Текущий том остался {ws.volume}; переключить позже — `ugar volume open {nxt}`.")
    return res
