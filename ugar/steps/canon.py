    ...
    def once() -> int:
        ...
        if llm:
            if report.errors:
                ...
                if prompts:
                    secho(f"⚠ API недоступен: промпты модельного слоя сохранены ({len(prompts)}) в logs/линтер_промпты/", fg=colors.YELLOW)
...
def snapshot(volume: int | None = None) -> Path:
    ...
    В канон снапшот вносит `ugar volume close N`. Возвращает путь черновика."""
    from .. import snapshot as snapshot_mod

    ws, cfg, lib = _ctx()
    volume = volume or ws.volume
    if volume != ws.volume:
        raise StepError(f"выгрузки — тома {ws.volume}; для среза тома {volume} переключитесь: `ugar volume open {volume}`.")
    ...
    echo("Внесите его в библиотеку правкой канона и `ugar canon-commit` (FR-K3 соблюдён).")
    return path
...
def rollback(chapter: int, to: str | None = None, yes: bool = False, confirm: Confirm | None = None) -> str:
    ...
    if st.state == "зафиксировано":
        ...
        try:
            ...
            secho(f"⚠ Откат выполнен, но выгрузки не пересчитаны: {e}. Поправьте канон и `ugar export`.", fg=colors.YELLOW)
        if to != "принято":
            st.rollback(to)
...
def retest(chapter: int = 1, fix: bool = False) -> Path:
    ...
    if fix:
        ...
        if green is not True:
            ...
            raise StepError(f"фиксация retest запрещена: {why} (FR-R3). Сначала `ugar regress` с непустым корпусом.")
        ...
        secho(f"Черновик записи журнала: retest/{stamp}/журнал_запись.md — внесите в 3.6 (сценарий Б).", fg=colors.GREEN)
        return ws.root / "retest" / stamp
    exporter.run_export(lib, ws.exports, ws.logs, ws.volume)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d")
...
def library_split(
    ...
    for line in plan.lines():
        echo(f"  {line}")
    if show:
        echo("Ничего не изменено (--показать). Выполнить: `ugar library-split`" + (" --с-историей" if with_history else "") + ".")
        return
...
def backup(
    ...
    ws, cfg, lib = _ctx()
    if not gitops.is_repo(lib):
        raise StepError("библиотека не под git — инициализируйте репозиторий (`ugar library-split` — как отдельный).")
    if add_remote:
        name, url = add_remote
        ...
        secho(f"Удалённое место «{name}» добавлено: {url}. Отправка — `ugar backup --push`.", fg=colors.GREEN)
    ...
    if len(remotes) < cfg.backup_remotes_min:
        secho("⚠ Добавьте удалённые репозитории/внешние копии (NFR-6): `ugar backup --добавить-remote <имя> <url|папка>`.",
              fg=colors.YELLOW)
    if gitops.dirty(lib):
        secho("⚠ В библиотеке незакоммиченные изменения (`ugar canon-commit`).", fg=colors.YELLOW)
