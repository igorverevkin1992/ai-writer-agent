"""Правки и решения автора: resolve (решения по самоволкам, FR-V2.5), edits (предпросмотр edits.md),
...
def resolve(chapter: int, flag_id: str | None = None, decision: str | None = None, registry: str | None = None) -> list:
    ...
    for r in resolutions:
        if r.flag_id == flag_id:
            ...
            return resolutions
    raise StepError(f"самоволка {flag_id} не найдена (см. `ugar resolve {chapter}`).")


def edits(chapter: int) -> list:
    """Предпросмотр правок: как парсер понял edits.md (без вызова Писателя). Возвращает правки."""
    ws, cfg, lib = _ctx()
    ...
    if bad:
        ...
        secho(f"Распознано {len(parsed)} правок. Далее: `ugar apply-edits {chapter}`.", fg=colors.GREEN)
    return parsed
...
def diff(chapter: int, k1: int | None = None, k2: int | None = None) -> list[str]:
    ...
    lines = list(difflib.unified_diff(a, b, f"draft_{k1}", f"draft_{k2}", lineterm="", n=1))
    if not lines:
        echo(f"draft_{k1} и draft_{k2} идентичны.")
        return lines
    for line in lines:
