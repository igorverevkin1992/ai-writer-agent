...
Ядро без typer: аргументы — обычные значения, вывод — stdout, ошибки — исключения ugar/errors.py.
"""
...
def compile(chapter: int) -> Path:  # noqa: A001 — имя команды `ugar compile`
...
def write(chapter: int, manual: bool = False, variants: int = 1, choose: str | None = None) -> int | None:
    """Отправить окно Писателю, сохранить draft_k.md (FR-W1). `variants` ≥ 2 — A/B (draft_k.md, draft_k.alt1.md …),
    `choose` — сделать вариант текущим draft_k.md (состояние не меняется). Возвращает номер черновика."""
    ws, cfg, lib = _ctx()
    ...
    if choose:
        ...
        secho(f"Вариант «{choose}» → draft_{st.draft}.md (прежний основной — alt0). Далее: `ugar verify1 {chapter}`.",
              fg=colors.GREEN)
        return st.draft
    ...
    if variants > 1:
        ...
        echo(f"Основной — draft_{k}.md; выбрать другой: `ugar write {chapter} --выбрать alt1`.")
    return k
...
def verify2(chapter: int, manual: bool = False, taste: bool = False, again: bool = False) -> list:
    ...
    if manual:
        ...
        echo(f"Принят ручной flags.json: {len(flags)} флагов.")
    else:
        try:
    ...
    if taste:
        try:
            ...
            echo(f"Вкус (совещательно, 02 §6.1): замечаний {len(advice)} → {ws.chapter_rel(chapter)}/taste.json")
        except adapters.ManualModeNeeded:
            echo(f"Промпт вкуса сохранён: {ws.chapter_rel(chapter)}/taste_prompt.md (ответ — в taste.json).")
        except ValueError as e:
...
def review(chapter: int) -> Path:
    """Пакет приёмки автора: review.md + edits.md + resolutions.json (FR-E1). Возвращает путь review.md."""
    ws, cfg, lib = _ctx()
    st = ChapterState(ws, chapter)
...
def apply_edits(chapter: int, manual: bool = False) -> int:
    ...
    secho(f"Правки внесены ({len(edits)} шт.: {how}) → draft_{new_k}.md. Далее: `ugar diff-check {chapter}`.",
          fg=colors.GREEN)
    return new_k
...
def diff_check(chapter: int, author_fix: bool = False, fragments: list[str] | None = None):
    ...
    if report.unauthorized:
        ...
        echo(f"Цикл повторяется: поправьте edits.md и выполните `ugar apply-edits {chapter}` (≤{cfg.edit_cycle_max_iterations} итераций).")
    elif report.not_applied:
        secho("Часть правок не внесена — повторите цикл.", fg=colors.YELLOW)
    else:
        secho(f"Дифф-контроль чист. Далее: `ugar accept {chapter}`.", fg=colors.GREEN)
    return report
...
def accept(chapter: int, yes: bool = False, confirm: Confirm | None = None) -> None:
    ...
    if unresolved:
        raise StepError(f"не решены самоволки: {', '.join(unresolved)} (resolutions.json).")
    ...
    secho(f"Глава {chapter} принята. Далее: `ugar canonize {chapter}`.", fg=colors.GREEN)
...
def canonize(
    ...
    if not apply:
        ...
        echo(f"Проверьте/поправьте пакет и примените: `ugar canonize {chapter} --apply`.")
        return path
    if not batch_path.exists():
        raise StepError(f"нет пакета canon_batch.md — сначала `ugar canonize {chapter}`.")
    ...
    if existing:
        ...
        return existing
    confirm_or_reject(yes, confirm, f"Применить пакет главы {chapter} к УГАР_Библиотека/ и закоммитить? (Д-8) (y)")
...
def run(chapter: int) -> None:
    ...
    while True:
        ...
        if state == "не-начато":
            ...
            review(chapter)
            echo("⏸ Пауза такта: заполните edits.md и resolutions.json, затем снова `ugar run N`.")
            return
            ...
            if data.get("not_applied") or data.get("unauthorized"):
                echo("⏸ Пауза такта: дифф-контроль не чист — решите и продолжите `ugar run N`.")
                return
        elif state == "дифф-контроль":
            echo(f"⏸ Пауза такта: приёмка автора — `ugar accept {chapter}`, затем `ugar run {chapter}`.")
            return
        elif state == "принято":
            if not (ws.chapter_dir(chapter) / "canon_batch.md").exists():
                canonize(chapter, apply=False, yes=False)
            echo(f"⏸ Пауза такта: подпишите пакет — `ugar canonize {chapter} --apply`.")
            return
