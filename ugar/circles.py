...
def _library_of(ws: Workspace, library: Path | None) -> Path | None:
    ...
    except (OSError, ValueError):  # без config.yaml или с битым — материал просто без темы
        return None
...
def build_material(ws: Workspace, scope: str, key: int | None = None, library: Path | None = None) -> tuple[str, str]:
    ...
    (13 §1), арки 2.5 целиком и события сетки с участниками сцен; `library` — для темы (иначе по config.yaml)."""
    ex = ws.exports
...
def run(ws: Workspace, cfg: Config, scope: str, chapter: int | None = None, only_missing: bool = True,
    ...
    Промпты аналитика (материал с тайнами и арками) — только в `круги_истории/промпты/`, Писателю не идут."""
    done: list[str] = []
    prompts: list[str] = []
    system = _template(ws)
    manual_reason = None
