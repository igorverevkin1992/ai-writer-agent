...
- `ugar status` — все главы: состояние, сводка флагов Э1/Э2 и колонка «Дальше»
  ...
  нерешённые самоволки, следующий шаг).
...
`dashboard [--открыть]` (FR-D1), `log` — последние API-вызовы с токенами и
стоимостью, `doctor` — диагностика установки, `regress [--llm]` и `add-golden`
...
```bash
python -m pytest tests/    # 84 теста
```
