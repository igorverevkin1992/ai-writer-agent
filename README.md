...
этапа 3 «полный такт без терминала» выполняется. Собранный интерфейс лежит в
данных пакета — Node автору не нужен; исходники — в `panel/` (Vite + React),
пересборка: `cd panel && npm install && npm run build`.
...
```bash
python -m pytest tests/    # 100 тестов
```
